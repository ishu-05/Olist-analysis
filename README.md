...
Full methodology, including every SQL technique used and the reasoning behind it, is documented in [``].
...
├── notebooks/
│   └── data_cleaning.ipynb      # Python/pandas cleaning + PostgreSQL load
├── sql/
...
This analysis is correlational, not causal — late deliveries are associated with lower review scores and modestly lower repeat-purchase rates, but the dataset cannot confirm delay as the sole cause. 
